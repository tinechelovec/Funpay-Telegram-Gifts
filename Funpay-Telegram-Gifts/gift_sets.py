...
def choose_existing_set_id(sets: Dict[str, GiftSet]) -> Optional[str]:
    ...
    sid = input_int("\nУкажите ID набора (0 — назад): ", min_val=MIN_SET_ID, allow_cancel=True)
    if sid is None:
        return None
    sid_s = str(sid)
...
def cmd_create_set():
    ...
    print(f"Вводите ID подарков по одному (1–{MAX_GIFT_KEY}). Нажмите Enter, чтобы закончить.")
    gift_keys: List[str] = []
    i = 1
    while i <= MAX_GIFT_KEY:
        s = input(f"Подарок №{i} — ID (1–{MAX_GIFT_KEY}, Enter — закончить, 0 — отмена): ").strip()
        if _is_cancel_token(s):
            print("Создание набора отменено.")
            press_enter()
            return
        ...
        if not s.isdigit() or s not in base:
            print(f"Укажите ID из списка (1–{MAX_GIFT_KEY}).")
            continue
        gift_keys.append(s)
        i += 1
    ...
    while True:
        set_id = input_int(
            f"\nУкажите ID набора (gift_tg для FunPay, число ≥ {MIN_SET_ID}, 0 — отмена): ",
        ...
        set_id_s = str(set_id)
        if set_id_s in base:
            print(f"ID {set_id_s} запрещён (занят подарком {base[set_id_s]['title']}). Выберите другой.")
            continue
        if set_id_s in sets:
...
def cmd_edit_set():
    ...
    if choice == 1:
        print_gifts_catalog(base)
        print(f"Вводите новые подарки по одному (1–{MAX_GIFT_KEY}). Enter — закончить.")
        gift_keys: List[str] = []
        i = 1
        cancelled = False
        while i <= MAX_GIFT_KEY:
        ...
        while True:
            new_id = input_int(
                f"Новый ID набора (число ≥ {MIN_SET_ID}, 0 — отмена): ",
                min_val=MIN_SET_ID,
                allow_cancel=True
            )
            if new_id is None:
            ...
            new_id_s = str(new_id)
            if new_id_s in base:
                print(f"ID {new_id_s} запрещён (занят подарком {base[new_id_s]['title']}). Выберите другой.")
                continue
